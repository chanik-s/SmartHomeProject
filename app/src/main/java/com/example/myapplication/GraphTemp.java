...
public class GraphTemp extends AppCompatActivity {
    ...
    private ArrayList<Sensor> arrayListTemp;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mchart = findViewById(R.id.TempChart);
        // 버튼 클릭 리스너 설정
        Button nextButton = findViewById(R.id.nextBtnT);
        Button prevButton=findViewById(R.id.prevBtnT);

        Intent intent = getIntent();
        arrayListTemp = (ArrayList<Sensor>) intent.getSerializableExtra("fortempgraph");
        ...
        nextButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (startIndex + 10 < arrayListTemp.size()) {
                    startIndex += 10;
                    updateChart();
                }
            }
    ...
    private void updateChart() {
        ...
        for (int i = startIndex; i < startIndex + 10 && i < arrayListTemp.size(); i++) {
            String dateStr = arrayListTemp.get(i).getTime();


            float value = Float.parseFloat(arrayListTemp.get(i).getTemperature());
            try {
        ...
        xAxis.setValueFormatter(new IndexAxisValueFormatter() {
            ...
            public String getFormattedValue(float value) {
                int index = (int) value;
                if (index >= 0 && index < arrayListTemp.size()) {
                    String dateStr = arrayListTemp.get(index).getTime();
                    try {
                        Date date = format.parse(dateStr);
                        assert date != null;
        ...
        //온도 최소 최고 정해줘야함!
        yAxisLeft.setAxisMinimum(-10f); // Y축 최소값 설정
        yAxisLeft.setAxisMaximum(60f); // Y축 최대값 설정
